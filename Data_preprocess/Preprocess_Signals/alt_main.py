...
def signal_preprocessing (ids, video_id):
       for id in ids:
           ...
           PS = pd.read_csv("/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/raw_data/Physiological_signals/cleaned_user" +str(id)+ "_physiological.csv",skiprows=1)
...
def extract_baseline_from_gap(annotation, PS, video1, video2):
    # Get start time of video 1
    v1_index = annotation[annotation["videoID"] == video1].index.max()

    # Get start time of video 2
    v2_index = annotation[annotation["videoID"] == video2].index.min()
    ...
    if v1_index is not None and v2_index is not None:
        ...
        print(f"\n🟦 Extracting baseline (Blue Screen) from {start_time} to {end_time}")

        # Filter physiological signal array
        PS_array = np.asanyarray(PS)
        baseline_data = []

        for i in range(len(PS_array)):
            ts = PS_array[i, 3]  # timestamp column
            if start_time <= ts <= end_time:
                baseline_data.append(PS_array[i])

        # Convert to DataFrame
        baseline_df = pd.DataFrame(baseline_data, columns=['time', 'GSR', 'HR', 'timestamp', 'time2'])

        # Normalize and compute baseline means
        baseline_GSR = (baseline_df['GSR'] - baseline_df['GSR'].min()) / (baseline_df['GSR'].max() - baseline_df['GSR'].min())
        baseline_HR = (baseline_df['HR'] - baseline_df['HR'].min()) / (baseline_df['HR'].max() - baseline_df['HR'].min())

        GSR_meanblue = baseline_GSR.mean()
        HR_meanblue = baseline_HR.mean()

        print(f"✅ Baseline GSR: {GSR_meanblue:.4f}, HR: {HR_meanblue:.4f}")

        return GSR_meanblue, HR_meanblue, baseline_df
    else:
        print("❌ Could not find valid indexes for video IDs.")
...
#                print("ID",id)
#                P_id  = P_id + 1
#                PS = pd.read_csv(f"/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/raw_data/Physiological_signals/user{id}_physiological.csv")
...
#                        df_check = pd.read_csv(f"/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/3_Normalized_window_data/{P_id}{v}_PS.csv")
#                        print(df_check[['GSRmean_diff', 'HRmean_diff']].head())


#                        print(new_window.dtypes)


#                        new_window.to_csv("/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/3_Normalized_window_data/"+str(P_id)+str(v)+"_PS.csv")
'''

def extract_features(ids, video_id):
    debug_file = open("debug_extract_features.txt", "w")
    ...
    for id in ids:
        ...
        PS = pd.read_csv(f"/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/raw_data/Physiological_signals/cleaned_user{id}_physiological.csv",skiprows=1)
        ...
        selected_rows2 = PS.iloc[600:651]
        selected_rows_GSR = (selected_rows2['GSR'] - selected_rows2['GSR'].min()) / (selected_rows2['GSR'].max() - selected_rows2['GSR'].min())
        selected_rows_HR = (selected_rows2['HR'] - selected_rows2['HR'].min()) / (selected_rows2['HR'].max() - selected_rows2['HR'].min())
        # GSR_meanblue = selected_rows_GSR.mean()
        # HR_meanblue = selected_rows_HR.mean()

        GSR_meanblue, HR_meanblue, baseline_df = extract_baseline_from_gap(annotation, PS, video1=1, video2=2)
        print("***********************************************************************************", file=debug_file)
        print("DEBUG: GSR_meanblue =", GSR_meanblue, "HR_meanblue =", HR_meanblue, file=debug_file)
        ...
        for v in video_id:
            ...
            temp_data["video_id"] = temp_data["video_id"].astype(int)  # Ensure type match
            original_data = temp_data[temp_data["video_id"] == v]
            data = original_data[['GSR', 'HR']].to_numpy()

            window_size = 50
            if len(data) < window_size:
                print(f"⚠️ Not enough data for windowing in user {id}, video {v}")
                continue
            ...
            # Assign valence/arousal based on video ID
            if v in [1, 2]:
                new_window["valence_acc_video"] = 1
                new_window["arousal_acc_video"] = 1
            elif v in [3, 4]:
            ...
            print("Sample of GSRmean_diff:", new_window["GSRmean_diff"].head().tolist(), file=debug_file)
    debug_file.close()
'''
def extract_features(ids, video_id):
...
#             score_file = f"/home/rounak/CODE/Low_Engagement_Detection/Data_preprocess/Preprocess_Signals/scores/scores/{id}{v}_scores.csv"
