import React, { useState, useEffect, useRef } from "react";
import { useLocation } from "react-router-dom";
import Container from "react-bootstrap/Container";
import Row from "react-bootstrap/Row";
import Col from "react-bootstrap/Col";
import NavbarComponent from "./NavbarComponent";
import ReactPlayer from "react-player";
import Modal from "react-bootstrap/Modal";
import Form from "react-bootstrap/Form";
import Button from "react-bootstrap/Button";
import FloatingLabel from "react-bootstrap/FloatingLabel";
import { CSVLink, CSVDownload } from "react-csv";
import { LinkContainer } from "react-router-bootstrap";
import { useReactMediaRecorder } from "react-media-recorder";
import FileSaver from "file-saver";

import vid0 from "./videos/videoMP4s/0.mp4";
import vid1 from "./videos/videoMP4s/1.mp4";
import vid2 from "./videos/videoMP4s/2.mp4";
import vid3 from "./videos/videoMP4s/3.mp4";
import vid4 from "./videos/videoMP4s/4.mp4";
import vid5 from "./videos/videoMP4s/5.mp4";
import vid6 from "./videos/videoMP4s/6.mp4";
import vid7 from "./videos/videoMP4s/7.mp4";
import vid8 from "./videos/videoMP4s/8.mp4";
import bits from "./assets/bits.png";

export default function VideoPlayer() {
  const location = useLocation();
  const { name, id } = location.state;

  const [currVideo, setCurrVideo] = useState(vid0);
  const [playing, setPlaying] = useState(true);
  const [playingID, setPlayingID] = useState(0);
  const [showModal, setShowModal] = useState(false);
  const [everythingDone, setEverythingDone] = useState(false);

  const canvasWidth = 300;
  const canvasHeight = 300;
  const jumpDistance = 10;

  const [form, setForm] = useState({
    valence: "",
    arousal: "",
    arousalValid: false,
    valenceValid: false,
  });

  const [locX, setLocX] = useState(canvasWidth / 2);
  const [locY, setLocY] = useState(canvasHeight / 2);

  const [val, setVal] = useState(5);
  const [aro, setAro] = useState(5);

  const [annotations, setAnnotations] = useState([
    ["time,valence,arousal,videoID"],
  ]);

  // deployment mode: predictions and timeline
  const [mode, setMode] = useState("collect"); // 'collect' or 'deploy'
  const [predictions, setPredictions] = useState([]); // array of {window_start_ms, window_end_ms, prob, pred}
  const [videoStartUnixMs, setVideoStartUnixMs] = useState(null);
  const playerRef = useRef(null);

  const { status, startRecording, stopRecording, mediaBlobUrl } =
    useReactMediaRecorder({ video: true, audio: true });
  const orderedList = [vid0, vid8, vid0, vid2, vid0, vid5, vid0, vid3, vid0, vid7, vid0, vid4, vid0, vid6, vid0, vid1];
  

  const handleShow = () => {
    setShowModal(true);
    setPlaying(false);
    setLocX(canvasWidth / 2);
    setLocY(canvasHeight / 2);
  };

  const handleSeek = () => {
    setPlaying(true);
    setShowModal(false);
  };

  const handleVideoEnded = () => {
    if (playingID === orderedList.length - 1) {
      finishUp();
    }
  
    stopRecording();
  
    setPlayingID(prev => {
      const next = (prev + 1) % orderedList.length;
      setCurrVideo(orderedList[next]);
      return next;
    });
  
    startRecording();
    setPlaying(true);
  };
  

  const finishUp = () => {
    setShowModal(false);
    setPlaying(false);
    setEverythingDone(true);
  };

  const getVideoID = (video) => {
    if (video === vid0) return 0;
    if (video === vid1) return 1;
    if (video === vid2) return 2;
    if (video === vid3) return 3;
    if (video === vid4) return 4;
    if (video === vid5) return 5;
    if (video === vid6) return 6;
    if (video === vid7) return 7;
    if (video === vid8) return 8;
    return -1;
  };

  // ---------------------------------------------------------------------------
  // DATA COLLECTION: update annotations when playing
  // ---------------------------------------------------------------------------
  useEffect(() => {
    if (playing) {
      setAnnotations((prev) => [
        ...prev,
        [Date.now(), val, aro, getVideoID(currVideo)],
      ]);
    }
  }, [playing, val, aro, currVideo]);

  // ---------------------------------------------------------------------------
  // DEPLOYMENT: poll prediction.json in deploy mode
  // ---------------------------------------------------------------------------
  useEffect(() => {
    if (mode !== "deploy") return;

    let mounted = true;

    const poll = async () => {
      try {
        const res = await fetch("/prediction.json", { cache: "no-store" });
        if (!res.ok) return;
        const data = await res.json(); // array of prediction windows
        if (!mounted) return;
        setPredictions(data);
      } catch (e) {
        // ignore while file not present or being written
      }
    };

    poll();
    const id = setInterval(poll, 1000);
    return () => {
      mounted = false;
      clearInterval(id);
    };
  }, [mode]);

  // ---------------------------------------------------------------------------
  // Map prediction windows → video timeline segments
  // ---------------------------------------------------------------------------
  const buildSegments = () => {
    if (!videoStartUnixMs || !playerRef.current) return [];
    const duration = playerRef.current.getDuration(); // seconds
    if (!duration || duration <= 0) return [];

    return predictions
      .map((p) => {
        const wStartSec = (p.window_start_ms - videoStartUnixMs) / 1000.0;
        const wEndSec = (p.window_end_ms - videoStartUnixMs) / 1000.0;
        const startPct = Math.max(0, Math.min(1, wStartSec / duration));
        const endPct = Math.max(0, Math.min(1, wEndSec / duration));
        return { startPct, endPct, pred: p.pred, prob: p.prob };
      })
      .filter((s) => s.endPct > 0 && s.startPct < 1);
  };

  const segments = buildSegments();

  const handleSubmitCSV = () => {
    const csv = annotations.map((row) => row.join(",")).join("\n");
    const csvData = new Blob([csv], { type: "text/csv;charset=utf-8;" });
    FileSaver.saveAs(csvData, "data.csv");
  };

  const downloadVideo = (blob_url) => {
    if (getVideoID(currVideo) === 0 && blob_url) {
      const a = document.createElement("a");
      a.style.display = "none";
      a.href = blob_url;
      a.download = `user_${id}_video_${getVideoID(currVideo)}.mp4`;
      document.body.appendChild(a);
      a.click();
      window.URL.revokeObjectURL(blob_url);
    }
  };

  const leftPressed = () => {
    if (locX > jumpDistance) setLocX(locX - jumpDistance);
    else setLocX(0);
  };
  const rightPressed = () => {
    if (locX < canvasWidth - jumpDistance) setLocX(locX + jumpDistance);
    else setLocX(canvasWidth);
  };
  const upPressed = () => {
    if (locY > jumpDistance) setLocY(locY - jumpDistance);
    else setLocY(0);
  };
  const downPressed = () => {
    if (locY < canvasHeight - jumpDistance) setLocY(locY + jumpDistance);
    else setLocY(canvasHeight);
  };

  const handleKeyDown = (e) => {
    e.preventDefault();
    if (e.key === "ArrowLeft") leftPressed();
    if (e.key === "ArrowRight") rightPressed();
    if (e.key === "ArrowUp") upPressed();
    if (e.key === "ArrowDown") downPressed();
  };

  const canvasRef = useRef(null);
  const canvasRef2 = useRef(null);

  useEffect(() => {
    const canvas = canvasRef.current;
    const context = canvas.getContext("2d");

    const canvas2 = canvasRef2.current;
    const context2 = canvas2.getContext("2d");

    // not drawing anything here; actual drawing happens in other useEffect
    // but refs are initialised
  }, []);

  const round = (value, decimals) =>
    Number(Math.round(value + "e" + decimals) + "e-" + decimals);

  const getArou = (x, y) => round(8 - (y / canvasWidth) * 8 + 1, 2);
  const getVal = (x, y) => round((x / canvasWidth) * 8 + 1, 2);

  useEffect(() => {
    const canvas = canvasRef.current;
    const context = canvas.getContext("2d");

    context.beginPath();
    context.fillStyle = "#DDDDDD";
    context.fillRect(0, 0, canvas.width, canvas.height);

    context.clearRect(0, 0, canvas.width, canvas.height);

    context.beginPath();
    context.arc(locX, locY, 6, 0, 2 * Math.PI, false);
    context.fillStyle = "red";
    context.fill();

    const canvas2 = canvasRef2.current;
    const context2 = canvas2.getContext("2d");

    let img = new Image();
    img.src = "/valArou.png";
    img.onload = function () {
      context2.drawImage(img, 0, 0, canvas.width, canvas.height);
    };

    setVal(getVal(locX, locY));
    setAro(getArou(locX, locY));
  }, [locX, locY]);

  useEffect(() => {
    if (mode === "collect") {
      // re-render circumplex canvas
      setLocX(x => x + 0.0001);
    }
  }, [mode]);
  
  return (
    <>
      <Container
        onKeyDown={handleKeyDown}
        tabIndex={0}
        style={{ outline: "none", transition: "1ms" }}
      >
        <NavbarComponent page="video" />

        <Row className="d-flex justify-content-center align-items-center">
          <Col md={8} lg={6} xs={12}>
            VIDEO PLAYER - User <b>{name}</b> ; PlayingID <b>{playingID}</b> ;
            Arousal - <b>{aro}</b> Valence - <b>{val}</b> ; Mode:{" "}
            <b>{mode}</b>
          </Col>
        </Row>

        <Row className="d-flex justify-content-center align-items-center">
          <Col md={10} lg={10} xs={12}>
            <ReactPlayer
              ref={playerRef}
              onSeek={() => {
                if (mode === "collect") handleSeek();
              }}
              onPause={() => {
                if (mode === "collect") handleShow();
              }}
              onPlay={() => {
                // when video starts/resumes, anchor the timeline to current wall-clock - current time
                const currentTimeSec = playerRef.current
                  ? playerRef.current.getCurrentTime()
                  : 0;
                setVideoStartUnixMs(Date.now() - currentTimeSec * 1000);
              }}
              url={currVideo}
              playing={playing}
              controls
              light={bits}
              width={`85%`}
              height={`auto`}
              onEnded={handleVideoEnded}
            />
          </Col>
        </Row>

        {/* Deployment-mode engagement timeline below player */}
        {mode === "deploy" && (
          <div
            style={{
              position: "relative",
              height: 12,
              width: "85%",
              margin: "0.5rem auto",
              background: "#eee",
              borderRadius: 4,
            }}
          >
            {segments.map((seg, idx) => {
              const left = `${seg.startPct * 100}%`;
              const width = `${Math.max(
                0.2,
                (seg.endPct - seg.startPct) * 100
              )}%`;
              const color = seg.pred === 1 ? "red" : "green";
              return (
                <div
                  key={idx}
                  title={`Prob: ${seg.prob.toFixed(2)}`}
                  style={{
                    position: "absolute",
                    left,
                    width,
                    top: 0,
                    bottom: 0,
                    background: color,
                    opacity: 0.7,
                    borderRadius: 2,
                  }}
                />
              );
            })}

            {/* moving playhead */}
            <div
              style={{
                position: "absolute",
                left: `${
                  (playerRef.current
                    ? playerRef.current.getCurrentTime() /
                      (playerRef.current.getDuration() || 1)
                    : 0) * 100
                }%`,
                width: 2,
                top: 0,
                bottom: 0,
                background: "#000",
                transform: "translateX(-1px)",
              }}
            />
          </div>
        )}

        <Row className="d-flex justify-content-center align-items-center">
            <div style={{ textAlign: 'left' }}>
                
                {mode === "collect" && (
                <Button
                    variant="primary"
                    onClick={handleSubmitCSV}
                    style={{ width: '200px', fontSize: '16px', marginRight: '10px' }}
                >
                    Download CSV of Annotations
                </Button>
                )}

                {/* Mode switch always visible */}
                <Button
                variant="secondary"
                onClick={() => setMode(mode === 'collect' ? 'deploy' : 'collect')}
                >
                Switch to {mode === 'collect' ? 'Deployment' : 'Collection'} UI
                </Button>
            </div>
        </Row>


        {/* Canvas quadrant UI */}
        {mode === "collect" && (
            <Row className="d-flex justify-content-center align-items-center">
            <Col>
                <div
                style={{
                    position: "fixed",
                    bottom: "480px",
                    right: "300px",
                }}
                >
                <canvas
                    ref={canvasRef2}
                    width={canvasWidth}
                    height={canvasHeight}
                    style={{ position: "absolute" }}
                />
                <canvas
                    ref={canvasRef}
                    width={canvasWidth}
                    height={canvasHeight}
                    style={{ position: "absolute" }}
                />
                </div>
            </Col>
            <Col>
                <div style={{ margin: "1vw", padding: "1vw" }}></div>
            </Col>
            </Row>
        )}
        <Row>
          <Col>
                {mode === "collect" && (
                    <>
                        {/* {(status === "idle" || status === "stopped") && (
                        <Button variant="primary" onClick={startRecording}>
                            Start Recording
                        </Button>
                        )} */}
                        {/* {status === "recording" && (
                        <Button variant="primary" onClick={stopRecording}>
                            Stop Recording
                        </Button>
                        )} */}
                        {status === "stopped" && (
                        <Button
                            variant="primary"
                            onClick={() => downloadVideo(mediaBlobUrl)}
                        >
                            Download Video
                        </Button>
                        )}
                    </>
                )}
          </Col>
        </Row>
      </Container>
    </>
  );
}
