...
def run_experiment(active_videos, threshold):
    all_videos = [1,2,3,4,5,6,7,8]

    # Testing is always 7 and 8
    test_videos = [7, 8]

    # Training = everything before the smallest active video
    min_active = min(active_videos)
    train_videos = list(range(1, min_active))

    # Active learning = whatever is passed in
    al_videos = active_videos

    print(f"Training videos: {train_videos}")
    print(f"Active Learning videos: {al_videos}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--active", type=str, required=True,
                        help="Comma separated active video ids, e.g. 2,3,4")
    parser.add_argument("--threshold", type=float, required=True)
    args = parser.parse_args()

    active_videos = list(map(int, args.active.split(",")))
    run_experiment(active_videos, args.threshold)
...
df_base = new_df[new_df["video_id"].isin(train_videos)]

# Active Learning Data → Flag 3, 4, 5, 6
df_active = new_df[new_df["video_id"].isin(active_videos)]


# Final Testing Data → Flag 7, 8
df_test = new_df[new_df["video_id"].isin(test_videos)]
...
X_testing = df_test.drop(['probe','video_id'], axis=1)
y_testing = df_test['probe']
...
for s in subject_id: # For user wise iteration Active
    ...
    # ------------------------------------------------------------------------------------------
    X_final=df_test.drop(['probe','video_id'], axis=1)
    y_final=df_test['probe']
    ...
    for i in range(0,active_X_test.shape[0]):
        ...
        test_row['prev_window'] = ypred.item()  # Extracts scalar value from NumPy array
    ...
    for i in range(0,final_X_test.shape[0]):
        ...
        test_row.loc[:, 'prev_window'] = ypred.item()  # Correctly assign previous window value
