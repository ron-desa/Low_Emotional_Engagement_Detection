...
print(f"Baseline videos: {baseline_videos}")
print(f"Active learning videos: {active_videos}")
...
for i in range(1,10):
    ...
    df_base = new_df[new_df["video_id"].isin(baseline_videos)]
    ...
    for s in subject_id: # For user wise iteration Active
        ...
        X=df_active.drop(['probe','video_id'], axis=1)
        y=df_active['probe']
        active_test_index = df_active["P_id"] == s

        # Keep video_id for reference
        active_video_ids = df_active.loc[active_test_index, "video_id"].values  
        ...
        for i in range(0,active_X_test.shape[0]):
            ...
            current_video_id = active_video_ids[i]
            ...
            test_row = test_row.drop(columns=['video_id'], errors='ignore')
            ...
            if ypred_prob < threshold:
                ...
                # Add user-probeed data to training set
