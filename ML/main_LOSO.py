...
for s in subject_id:
    ...
    # columns = ["Score", "GSRmean_persen_diff", "HRmean_persen_diff", "valence_acc_video", "arousal_acc_video", 'prev_window2']
...
# --------------------------------------------------------------------------------------------
    ...
    #     print(f"--- Predicting row {i}/{X_test.shape[0]} for User {s} ---")

    #     test_row=X_test.iloc[[i]].copy()  # Double brackets keep it as a DataFrame
    #     y_test_cur= y_test.iloc[i]
    ...
    #     print("--------------------------------------------------------------------------")
    #     with open(f'LOSO/:y_test_vs_y_pred_comparison{threshold}.csv', 'a', newline='') as csvfile:
    #         writer = csv.writer(csvfile)
    #         writer.writerow([s, y_test_cur, ypred])
